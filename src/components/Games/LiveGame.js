...
export default class LiveGame extends React.Component {
                ...
                <Grid.Row stretched className="clear">
                    ...
                    <Grid.Column width={4} className="game-control-box">
                        <div className="game-control-content">
                            ...
                            {state !== 3 && <button className="game-finish-button" onClick={this.finishGame}>Finish Game</button>}
                            {state === 3 && <button className="game-finish-button" onClick={()=>{window.location.href="/home" }}>Exit</button>}
    ...
    getGameDetails(headers) {
        fetch(`http://localhost:3000/game/` + this.state.lobby_id + '/details', {
            ...
            .then(result => result.json())
            .then(game_details => {
                this.setState({ teams: game_details.game.teams, goals: game_details.game.goals.reverse(), state: game_details.game.state });
                if (game_details.finish_date === null)
                    this.setState({ game_details: { ...game_details.game, finish_date: null } });
                else
    ...
    finishGame(e) {
        ...
        fetch('http://127.0.0.1:3000/game/' + this.state.lobby_id + '/finish', {
            ...
            if (data.status === 200) {
                ...
                let aux=self.state.game_details;
                aux.finish_date=Date.now();
                self.setState({game_details: aux});
            }
